...
%     saved in the indexed field Tp_i.parameterVariation, which contains
%     the following information:
%           Tp_i.parameterVariation{ss}.labelShort - (short) string telling us which parameter is varied
%           Tp_i.parameterVariation{ss}.label - string telling us which parameter is varied
%           Tp_i.parameterVariation{ss}.value - value of the parameter (e.g. -20% in dlens)
%           Tp_i.parameterVariation{ss}.adjValue - actual lens density value corresponding to the change in Tp{}.parameterVariation.value
%
...
%     adjusted. This scheme also helps with issues with multi-level
%     indexing.
%
% Input:
...
whichParameter = p.Results.whichParameter;

% Sample
for ii = 1:NTitrations
    switch whichParameter
        case 'dlens' % Lens density
            parv = linspace(SSTGetDefaultParameterRange(whichParameter, 'start'), SSTGetDefaultParameterRange(whichParameter, 'end'), NTitrations);
    ...
    obj.Tp_i{theIdx}.parameterVariation.labelShort = parvlabel;
    obj.Tp_i{theIdx}.parameterVariation.label = parvlabellong;
    obj.Tp_i{theIdx}.parameterVariation.value = parv;
    obj.Tp_i{theIdx}.parameterVariation.adjValue = parvreal;
end
