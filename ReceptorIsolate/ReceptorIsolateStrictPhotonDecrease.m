function [isolatingPrimary] = ReceptorIsolateStrictPhotonDecrease(T_receptors,whichReceptorsToIsolate,whichReceptorsToIgnore,B_primary,backgroundPrimary,initialPrimary,whichPrimariesToPin,primaryHeadRoom,maxPowerDiff,desiredContrasts,ambientSpd)
% [isolatingPrimaries] = ReceptorIsolateStrictPhotonDecrease(T_receptors,whichReceptorsToIsolate,whichReceptorsToIgnore,B_primary,backgroundPrimary,initialPrimary,whichPrimariesToPin,lambdas,primaryHeadRoom,[desiredContrasts],[ambientSpd])
%
% Find the best isolating modulation around a given background. This one
% enforces the isolating primaries to be DECREMENTS.
%
...
vub = [initialPrimary(whichPrimariesToPin) ; backgroundPrimary(whichPrimariesToVary)];
vlb = vlb(sortIndex);
vub = vub(sortIndex);
